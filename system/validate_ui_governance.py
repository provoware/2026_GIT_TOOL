...
ALLOWED_SOURCE_STATUSES = {"authoritative", "hardened", "protected", "feature_local"}
...
def _list(value: Any, label: str, *, nonempty: bool = True) -> list[Any]:
    if not isinstance(value, list) or (nonempty and not value):
        suffix = "eine nichtleere Liste" if nonempty else "eine Liste"
        raise UiGovernanceError(f"{label} muss {suffix} sein.")
    return value
...
def validate_policy(policy: Mapping[str, Any], *, root: Path = ROOT) -> Mapping[str, Any]:
    ...
    _unique_texts(
        ...
        "principles.single_consumer_exceptions",
    )

    sources = _list(data["authoritative_sources"], "authoritative_sources")
    ...
    for index, raw in enumerate(sources):
        ...
        source_owners[responsibility] = owner
    ...
    if source_owners.get("design_tokens") != "config/design-tokens.json":
        raise UiGovernanceError("config/design-tokens.json muss die autoritative Tokenquelle bleiben.")

    transitional = _list(data["transitional_sources"], "transitional_sources")
    transitional_paths: list[str] = []
    for index, raw in enumerate(transitional):
        ...
        target_block = _positive_int(item.get("target_block"), f"transitional_sources[{index}].target_block")
    ...
    for index, path in enumerate(forbidden):
        resolved = _repo_path(root, path, f"forbidden_parallel_sources[{index}]", must_exist=False)
        if resolved.exists():
            raise UiGovernanceError(f"Verbotene Parallelquelle existiert: {path}")

    planned = _list(data["planned_responsibilities"], "planned_responsibilities")
    ...
    for index, raw in enumerate(planned):
        ...
        target_block = _positive_int(item.get("target_block"), f"planned_responsibilities[{index}].target_block")
        if target_block <= current_block:
        ...
        if condition != "always":
            consumer_paths = _unique_texts(
                _list(consumers, f"planned_responsibilities[{index}].current_consumers"),
            ...
            for consumer_index, consumer in enumerate(consumer_paths):
                _repo_path(
                    root,
                    consumer,
                    f"planned_responsibilities[{index}].current_consumers[{consumer_index}]",
                    must_exist=True,
                )
        if target in forbidden:
            raise UiGovernanceError(f"Geplantes Ziel ist zugleich verboten: {target}")
        planned_ids.append(responsibility)
        planned_targets.append(target)
    if len(set(planned_ids)) != len(planned_ids):
        raise UiGovernanceError("Geplante Verantwortungen sind nicht eindeutig.")
    if len(set(planned_targets)) != len(planned_targets):
        raise UiGovernanceError("Geplante Zielpfade sind nicht eindeutig.")
    if "system/ui_tokens.py" in planned_targets:
        raise UiGovernanceError("Ein handgepflegtes system/ui_tokens.py ist nicht zulässig.")

    contracts = _list(data["protected_contracts"], "protected_contracts")
    ...
    for index, raw in enumerate(contracts):
        ...
        for evidence_index, evidence_path in enumerate(evidence):
            _repo_path(
                root,
                evidence_path,
                f"protected_contracts[{index}].evidence[{evidence_index}]",
                must_exist=True,
            )
    ...
    duplicates = _list(data["duplication_register"], "duplication_register")
    ...
    for index, raw in enumerate(duplicates):
        ...
        for location_index, location in enumerate(locations):
            _repo_path(
                root,
                location,
                f"duplication_register[{index}].locations[{location_index}]",
                must_exist=True,
            )
        _text(item.get("decision"), f"duplication_register[{index}].decision")
        target_block = _positive_int(item.get("target_block"), f"duplication_register[{index}].target_block")
        if target_block <= current_block:
    ...
    if len(set(duplicate_ids)) != len(duplicate_ids):
        raise UiGovernanceError("Duplikat-IDs sind nicht eindeutig.")
    if len(set(duplicate_topics)) != len(duplicate_topics):
        raise UiGovernanceError("Duplikat-Themen sind nicht eindeutig.")

    _unique_texts(_list(data["non_goals"], "non_goals"), "non_goals")

    criteria = _list(data["acceptance_criteria"], "acceptance_criteria")
    ...
    migration = _list(data["migration_order"], "migration_order")
    blocks: list[int] = []
    for index, raw in enumerate(migration):
        item = _mapping(raw, f"migration_order[{index}]")
        block = _positive_int(item.get("block"), f"migration_order[{index}].block")
        if block <= current_block:
            raise UiGovernanceError("Migrationsblöcke müssen nach Block 1 liegen.")
    ...
    if next_block != blocks[0] or next_block != current_block + 1:
        raise UiGovernanceError("Der nächste zulässige Block muss unmittelbar auf Block 1 folgen.")

    return data


def validate_changed_paths(paths: Iterable[str], *, allowed: frozenset[str] = BLOCK_1_ALLOWED_PATHS) -> list[str]:
    normalized: list[str] = []
    for index, value in enumerate(paths):
        ...
        if text not in allowed:
            raise UiGovernanceError(f"Block 1 darf diese Datei nicht ändern: {text}")
...
def main() -> int:
    ...
    print(
        ...
        f"{len(policy['duplication_register'])} Duplikatentscheidungen, "
