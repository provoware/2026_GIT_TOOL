...
MODULE_ACTIONS: dict[str, list[dict[str, Any]]] = {
    ...
    "beispiel_modul": [
        _action("echo", "Beispiel ausführen", fields=[_field("text", "Text", default="Hallo")])
    ],
    "todo_kalender": [
        ...
        _action(
            ...
            "Kalender laden",
            fields=[
                _field("view", "Ansicht", "select", options=["jahr", "monat", "woche"], default="monat"),
        ...
        _action(
            ...
            "Aufgabe erledigen",
            mode="write",
            fields=[_field("id", "Aufgaben-ID", required=True), _field("done_date", "Erledigt am", "date")],
        ),
    ...
    "notiz_editor": [
        ...
        _action("toggle_favorite", "Favorit umschalten", mode="write", fields=[_field("id", "Notiz-ID", required=True)]),
    ],
    "charakter_modul": [
        ...
        _action(
            ...
            mode="write",
            fields=[_field("id", "Charakter-ID", required=True), _field("name", "Name"), _field("role", "Rolle"), _field("archetype", "Archetyp")],
        ),
        _action("toggle_favorite", "Favorit umschalten", mode="write", fields=[_field("id", "Charakter-ID", required=True)]),
    ],
    "download_aufraeumen": [
        _action(
            "scan",
        ...
        _action("build_plan", "Aufräumplan erstellen", fields=[_field("items", "Scan-Ergebnis (JSON)", "json", required=True), _field("selected", "Ausgewählte Pfade", "list")]),
        _action(
            "apply_plan",
            "Aufräumplan anwenden",
            mode="write",
            fields=[_field("operations", "Operationen (JSON)", "json", required=True), _field("dry_run", "Nur simulieren", "checkbox", default=True)],
            confirm="Der Aufräumplan kann Dateien verschieben. Fortfahren?",
        ),
        _action("undo", "Letzte Aktion rückgängig", mode="write", confirm="Letzte Aufräumaktion wirklich rückgängig machen?"),
    ...
    "datei_suche": [
        ...
        _action("undo", "Letzte Organisation rückgängig", mode="write", confirm="Letzte Dateiaktion rückgängig machen?"),
        _action("history", "Historie laden"),
    ],
    "datei_manager": [
        ...
        _action(
            ...
            "Dateien taggen",
            mode="write",
            fields=[_field("paths", "Dateipfade", "list", required=True), _field("tags", "Tags", "list", required=True)],
        ),
        _action("toggle_favorite", "Favorit umschalten", mode="write", fields=[_field("path", "Dateipfad", "path", required=True)]),
    ],
    "archiv_manager": [
        _action("list_archives", "Archive laden"),
        _action("list_entries", "Einträge laden", fields=[_field("archive", "Archivkennung", required=True), _field("query", "Suche"), _field("category", "Kategorie")]),
        _action("add_entries", "Einträge hinzufügen", mode="write", fields=[_field("archive", "Archivkennung", required=True), _field("value", "Inhalt", "textarea", required=True), _field("category", "Kategorie", default="Allgemein"), _field("apply_spelling", "Rechtschreibvorschläge anwenden", "checkbox", default=False)]),
        _action("create_archive", "Archiv anlegen", mode="write", fields=[_field("name", "Name", required=True), _field("description", "Beschreibung", "textarea"), _field("split_on_comma", "Kommas trennen Einträge", "checkbox", default=True)]),
        _action("update_entry", "Eintrag bearbeiten", mode="write", fields=[_field("entry_id", "Eintrags-ID", "number", required=True), _field("value", "Inhalt", "textarea", required=True), _field("category", "Kategorie", default="Allgemein")]),
        _action("delete_entry", "Eintrag löschen", mode="write", fields=[_field("entry_id", "Eintrags-ID", "number", required=True)], confirm="Archiveintrag wirklich löschen?"),
    ],
    "media_wavesurfer": [
        _action("list_features", "Funktionen laden"),
        _action("list_markers", "Marker laden"),
        _action("list_regions", "Regionen laden"),
        _action("add_marker", "Marker setzen", mode="write", fields=[_field("time", "Zeit in Sekunden", "number", required=True), _field("label", "Bezeichnung"), _field("color", "Farbe", "color", default="#38bdf8")]),
        _action("add_region", "Region anlegen", mode="write", fields=[_field("start", "Start", "number", required=True), _field("end", "Ende", "number", required=True), _field("label", "Bezeichnung"), _field("color", "Farbe", "color", default="#22c55e")]),
        _action("set_minimap", "Minimap anpassen", mode="write", fields=[_field("height", "Höhe", "number", default=48), _field("zoom", "Zoom", "number", default=1), _field("color", "Farbe", "color", default="#38bdf8")]),
        _action("export_profile", "Exportprofil wählen", mode="write", fields=[_field("profile_id", "Profil", "select", options=["wav_44100", "mp3_high"], required=True)]),
    ],
    "media_ffmpeg_wrapper": [
        _action("list_presets", "Presets laden"),
        _action("build_command", "FFmpeg-Befehl erzeugen", mode="write", fields=[_field("input_path", "Eingabedatei", "path", required=True), _field("preset_id", "Preset", "select", options=["mp3_standard", "wav_master"], required=True), _field("output_name", "Ausgabename")]),
        _action("simulate_progress", "Fortschritt simulieren", mode="write", fields=[_field("job_id", "Job-ID", required=True)]),
    ...
    "profil_manager": [
        ...
        _action("create_profile", "Profil anlegen", mode="write", fields=[_field("name", "Profilname", required=True)]),
        _action("set_active", "Profil aktivieren", mode="write", fields=[_field("name", "Profilname", required=True)]),
        _action("rename_profile", "Profil umbenennen", mode="write", fields=[_field("from", "Bisheriger Name", required=True), _field("to", "Neuer Name", required=True)]),
        _action("delete_profile", "Profil löschen", mode="write", fields=[_field("name", "Profilname", required=True)], confirm="Profil und zugehörigen Projektordner wirklich löschen?"),
    ],
}
...
class WebModuleBridge:
    ...
    def invoke(self, module_id: str, action_id: str, payload: Mapping[str, Any] | None = None) -> dict[str, Any]:
        with self._lock:
            ...
            if action_id not in allowed:
                raise WebModuleBridgeError(f"Aktion ist für {module_id} nicht freigegeben: {action_id}")
    ...
    def snapshots(self) -> dict[str, Any]:
        ...
        for module_id, action_id in DEFAULT_ACTIONS.items():
            ...
            try:
                ...
                snapshots[module_id] = {"status": "error", "message": str(exc), "data": {}}
        return snapshots
    ...
    def _load_module(self, module_id: str, descriptor: Mapping[str, Any]) -> Any:
        ...
        try:
            ...
            raise WebModuleBridgeError(f"Modul kann nicht geladen werden: {module_id}: {exc}") from exc
