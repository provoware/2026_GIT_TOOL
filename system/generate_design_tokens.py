#!/usr/bin/env python3
"""Generate all derived design-token artifacts from config/design-tokens.json."""
...
def load_tokens() -> dict[str, Any]:
    ...
    required = {"meta", "themes", "spacing", "typography", "layout"}
    missing = sorted(required.difference(data))
    if missing:
        raise ValueError(f"Fehlende Token-Gruppen: {', '.join(missing)}")
    if data["meta"].get("defaultTheme") not in data["themes"]:
...
def build_css(tokens: dict[str, Any]) -> str:
    ...
    shared = {k: v for k, v in tokens.items() if k not in {"meta", "themes", "$schemaVersion"}}
...
def expected_outputs(tokens: dict[str, Any]) -> dict[Path, str]:
    return {
        ...
        GENERATED / "design-tokens-webmanifest.json": json.dumps(build_webmanifest(tokens), indent=2, ensure_ascii=False) + "\n",
        GENERATED / "design-tokens-module-manifest.json": json.dumps(build_module_manifest(tokens), indent=2, ensure_ascii=False) + "\n",
...
def main() -> int:
    ...
    parser.add_argument("--check", action="store_true", help="Nur prüfen, ob alle Ausgaben aktuell sind")
    ...
    if stale:
        ...
        for path in stale:
            print(f"- {path}")
        return 1
    print("Design-Token-Ausgaben sind aktuell." if args.check else "Design-Token-Ausgaben wurden erzeugt.")
    return 0
